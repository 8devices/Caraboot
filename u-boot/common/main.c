...
# else
				key_press=getc();  /* consume input	*/			  	  
				if (key_press == '\x1B'){ // if pressed <ESC>
					abort  = 1;	/* don't auto boot	*/
					bootdelay = 0;	/* no more delay	*/
				}
# endif	
...
# ifndef CFG_HUSH_PARSER
		int autoboot_return = run_command (s, 0);
# else
		int autoboot_return = parse_string_outer(s, FLAG_PARSE_SEMICOLON |
				    FLAG_EXIT_FROM_LOOP);
# endif

...
#warning CONFIG_SYS_MALLOC_LEN may be too small for EHCI
	...
	ehci_writel(&ctrl->hcor->or_asynclistaddr, PHYSADDR((uint32_t)&ctrl->qh_list));
...
int usb_lowlevel_init(int index, void **controller)
{
	...
	/* Set async. queue head pointer. */
	ehci_writel(&ehcic[index].hcor->or_asynclistaddr, PHYSADDR((uint32_t)qh_list));
	...
	ehci_writel(&ehcic[index].hcor->or_periodiclistbase,
		PHYSADDR((uint32_t)ehcic[index].periodic_list));
